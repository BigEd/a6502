 ...
 * (as inherited from the libopencm3 project.)
 *
...
/* original demo code does a loopback test in this callback.
   We prefer to place input in a global buffer
   Might be nice not even to use a callback, but seems necessary.
 */
	if (len)
		usb_rx_buf[0]=buf[0];

/*
        int len2 = sprintf(&buf[len], "%d < ", len);
	if (len) {
		while (usbd_ep_write_packet(0x82, buf, len+len2) == 0)
			;
	}
 */

/* is this meant to be an LED signal? it seems to have no effect
